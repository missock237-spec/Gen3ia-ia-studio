...
function PurchasesContent() {
  ...
  return (
    <main className="min-h-screen bg-[#f6f4ef] px-4 py-6 text-neutral-900 sm:px-6 lg:px-10">
              ...
              <div className="mt-5 space-y-3">
                {licenses.length === 0 ? <p className="rounded-2xl border border-dashed border-neutral-300 p-6 text-center text-sm text-neutral-400">Aucune licence enregistrée.</p> : licenses.map((license) => (
                  <div key={license.id} className="rounded-2xl border border-neutral-200 bg-neutral-50 p-4">
                    <div className="flex items-start justify-between gap-3"><div><p className="font-medium">{license.extension?.name ?? license.extensionId}</p><p className="mt-1 text-xs text-neutral-400">{license.extension?.developerName ?? "Développeur"}</p></div><span className={`rounded-full px-2.5 py-1 text-[10px] ${license.status === "active" ? "bg-emerald-100 text-emerald-600" : "bg-neutral-100 text-neutral-500"}`}>{license.status}</span></div>
                    <div className="mt-3 grid grid-cols-2 gap-2 text-[11px] text-neutral-400"><span>Créée<br /><b className="text-neutral-600">{date(license.createdAt)}</b></span><span>Expiration<br /><b className="text-neutral-600">{license.expiresAt ? date(license.expiresAt) : "Sans expiration"}</b></span></div>
                    <div className="mt-3 rounded-xl border border-neutral-200 bg-white px-3 py-2 font-mono text-[10px] text-neutral-500 break-all">{license.licenseKey}</div>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section className="rounded-3xl border border-[rgba(23,23,20,0.09)] bg-white p-5 shadow-[0_2px_10px_rgba(15,23,42,0.05)]">
              ...
              <div className="mt-5 space-y-3">
                {purchases.length === 0 ? <p className="rounded-2xl border border-dashed border-neutral-300 p-6 text-center text-sm text-neutral-400">Aucun achat enregistré.</p> : purchases.map((purchase) => (
                  <div key={purchase.id} className="rounded-2xl border border-neutral-200 bg-neutral-50 p-4">
                    <div className="flex items-start justify-between gap-3"><div><p className="font-medium">{purchase.extension?.name ?? purchase.extensionId}</p><p className="mt-1 text-xs text-neutral-400">{purchase.kind === "subscription" ? "Abonnement" : "Achat unique"} · {purchase.provider === "chariow" ? "Chariow" : "Wallet"}</p></div><b className="text-sm">{money(purchase.amountMinor, purchase.currency)}</b></div>
                    <div className="mt-3 flex items-center justify-between text-[11px] text-neutral-400"><span>{date(purchase.paidAt ?? purchase.createdAt)}</span><span className={`rounded-full px-2.5 py-1 ${purchase.status === "paid" ? "bg-emerald-100 text-emerald-600" : purchase.status === "refunded" ? "bg-red-50 text-red-600" : "bg-neutral-100 text-neutral-500"}`}>{purchase.status}</span></div>
                  </div>
                ))}
              </div>
            </section>
