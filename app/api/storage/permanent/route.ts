...
export async function POST(request: NextRequest) {
  ...
  } catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "File upload failed" }, { status: 400 }); }
}
...
export async function DELETE(request: NextRequest) {
  ...
  catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "File delete failed" }, { status: 400 }); }
}
