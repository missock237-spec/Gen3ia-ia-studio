...
export default function StudioPage() {
  ...
  useEffect(() => onAuthStateChanged(auth, async (current) => {
    setUser(current);
    // L'etat Firebase client peut etre perdu (webviews mobiles) : les appels
    // partent avec authFetch, qui utilise le cookie de session serveur.
    const [ads, memory, storage] = await Promise.all([authFetch("/api/ads/connections", { cache: "no-store" }), authFetch("/api/memory", { cache: "no-store" }), authFetch("/api/storage/permanent", { cache: "no-store" })]);
  ...
  const generateAd = async () => {
    if (sessionDisponible === false) { setMessage("Session expirée. Reconnectez-vous."); return; }
    if (objective.trim().length < 10) return; setBusy(true); setMessage("");
