...
async function executeAction(rawAction: unknown) {
  ...
  switch (action.type) {
    ...
    case "keyboard.key": { const key = keyFromString(action.key); if (!key) throw new Error(`Unsupported keyboard key: ${action.key}`); await keyboard.pressKey(key); return; }
...
function connect() {
  ...
  socket.on("message", async (raw) => {
    try {
      ...
      if (message.type === "resume") { paused = false; startCapture(); return; }
      if (message.type === "stop") { paused = true; stopped = true; stopCapture(); if (heartbeatTimer) clearInterval(heartbeatTimer); heartbeatTimer = undefined; console.warn(`Gen3ia Live Agent stopped: ${String(message.reason || "No reason provided")}`); }
