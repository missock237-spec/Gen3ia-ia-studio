...
if (parsedGatewayUrl.protocol !== "wss:" && !(isLoopback && parsedGatewayUrl.protocol === "ws:")) {
  throw new Error("Gen3ia Live requires wss:// in non-local environments.");
}
...
async function loadActionJournal() {
  try {
    ...
    completedActions = new Map(
      parsed.actions
        .filter((item) => typeof item?.actionId === "string" && z.string().uuid().safeParse(item.actionId).success)
...
async function emergencyStopRequested(): Promise<boolean> {
  try {
    await readFile(emergencyStopFile);
    return true;
  } catch {
    return false;
  }
    ...
    try {
      ...
      if (message.type === "action") {
        ...
        if (!z.string().uuid().safeParse(actionId).success) {
          throw new Error("Gateway returned an invalid action identifier.");
        }
        ...
        try {
          ...
          if (completedActions.size > MAX_COMPLETED_ACTIONS) {
            completedActions.delete(completedActions.keys().next().value!);
          }
          try {
            await persistActionJournal();
          } catch (error) {
            console.error("failed to persist live action journal", error);
          }
          send({ type: "action.result", sessionId, actionId, ok: true });
        } catch (error) {
          send({
      ...
      if (message.type === "resume") {
        if (await emergencyStopRequested()) {
          activateEmergencyStop("Local stop file is present.");
          return;
        }
        paused = false;
        startCapture();
      ...
      if (message.type === "stop") {
        activateEmergencyStop(String(message.reason || "No reason provided"));
      }
    } catch (error) {
...
emergencyStopTimer = setInterval(() => {
  void emergencyStopRequested().then((requested) => {
    if (requested) activateEmergencyStop("Local stop file is present.");
  });
...
void loadActionJournal()
  ...
  .catch((error) => {
    console.error("failed to initialize live action journal", error);
    process.exit(1);
  });
