...
export interface OrchestratorResult {
  executionId: string;
  roles: AgentRole[];
  state: Awaited<ReturnType<AgentRuntime["run"]>>;
  summary: string;
}
...
const READ_POLICY: ExecutionPolicy = {
  ...
  permissions: ["tool.read", "file.read", "network.read"],
  allowNetwork: true,
};

const AGENTS: Record<AgentRole, AgentDefinition> = {
  customer_service: {
    id: "customer_service", name: "Customer Service Agent",
    mission: "Gérer les demandes clients, FAQ, réclamations, qualification et escalade humaine.",
    capabilities: ["multilingual support", "FAQ", "customer context", "triage", "human escalation"], policy: READ_POLICY,
  },
  sales: {
    id: "sales", name: "Sales Agent",
    mission: "Qualifier les prospects, préparer les suivis, rendez-vous, offres et recommandations commerciales.",
    capabilities: ["lead qualification", "follow-up", "appointment preparation", "offer drafting", "pipeline analysis"], policy: READ_POLICY,
  },
  content: {
    id: "content", name: "Content Agent",
    mission: "Produire et décliner des contenus marketing pour TikTok, Instagram, SEO, newsletters et campagnes.",
    capabilities: ["social content", "SEO", "short-video scripts", "newsletter", "content calendar"], policy: READ_POLICY,
  },
  admin: {
    id: "admin", name: "Admin Agent",
    mission: "Automatiser et analyser les opérations administratives, planning, reporting et données structurées.",
    capabilities: ["reports", "scheduling", "invoice preparation", "inventory alerts", "spreadsheet analysis"], policy: READ_POLICY,
  },
  analytics: {
    id: "analytics", name: "Business Intelligence Agent",
    mission: "Analyser marché, concurrence, tendances, performances et opportunités afin d'aider à la décision.",
    capabilities: ["research", "competitor analysis", "trend analysis", "forecasting", "recommendations"], policy: READ_POLICY,
  },
};
...
function buildSteps(task: OrchestratorTask, roles: AgentRole[], executionId: string): RuntimeStep[] {
  const context = task.context ?? {};
  const steps: RuntimeStep[] = roles.map((role, index) => {
    const definition = AGENTS[role];
    const previous = index > 0 ? [`agent-${roles[index - 1]}`] : [];
    return {
      id: `agent-${role}`,
      type: role === "analytics" ? "research" : "llm",
      name: definition.name,
      description: `${definition.mission} Travaille comme membre d'une équipe multi-agent. Retourne des résultats structurés et directement exploitables par les agents suivants. Capacités: ${definition.capabilities.join(", ")}.`,
      dependencies: previous,
      status: "pending",
      input: { context, role, instruction: task.objective },
      toolName: role === "analytics" ? "web.search" : undefined,
      skillIds: [], maxRetries: 2, timeoutMs: 120_000,
      sideEffect: false, requiresApproval: false, agentRole: role,
    };
  });
  steps.push({
    id: "orchestrator-synthesis", type: "llm", name: "Orchestrator Synthesis",
    description: "Fusionner les sorties de toute l'équipe en une réponse opérationnelle unique. Identifier les décisions, prochaines actions, dépendances et points nécessitant une validation humaine.",
...
export function createOrchestratorPlan(task: OrchestratorTask): { executionId: string; roles: AgentRole[]; plan: RuntimePlan } {
  ...
  return {
    executionId, roles,
    plan: { executionId, objective: task.objective, steps: buildSteps(task, roles, executionId), maxConcurrency: Math.min(4, roles.length), maxIterations: 20 },
  };
}
...
export async function runOrchestrator(task: OrchestratorTask): Promise<OrchestratorResult> {
  ...
  const policy: ExecutionPolicy = {
    ...
    allowedTools: ["web.search", "file.read"],
    permissions: ["tool.read", "file.read", "network.read"],
    maxSteps: Math.max(DEFAULT_EXECUTION_POLICY.maxSteps, plan.steps.length + 5),
    allowNetwork: true,
  };
