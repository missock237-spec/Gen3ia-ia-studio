...
export type RuntimeStepStatus = z.infer<
  typeof RuntimeStepStatusSchema
>;
...
export type RuntimeStepType = z.infer<
  typeof RuntimeStepTypeSchema
>;

export const RuntimeStepSchema = z.object({
  id: z.string().min(1),
  type: RuntimeStepTypeSchema,
  name: z.string().min(1),
  description: z.string().min(1),

  dependencies: z.array(z.string()).default([]),

  status: RuntimeStepStatusSchema.default("pending"),
...
export type RuntimeStep = z.infer<
  typeof RuntimeStepSchema
>;
...
export const RuntimePlanSchema = z.object({
  ...
  maxIterations: z.number().int().positive().max(50).default(10),
});

export type RuntimePlan = z.infer<
  typeof RuntimePlanSchema
>;
