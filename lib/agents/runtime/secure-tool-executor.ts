...
export async function executeToolSecurely(options: SecureToolExecutionOptions): Promise<unknown> {
  ...
  try {
    ...
    if (options.toolName === "ads.publish") {
      ...
      if (typeof options.input.key !== "string") throw new Error("memory.read requires key"); result = await recall({ userId: options.userId, key: options.input.key });
    } else if (options.toolName === "memory.write") {
      if (typeof options.input.key !== "string") throw new Error("memory.write requires key"); await remember({ userId: options.userId, key: options.input.key, value: options.input.value, source: "agent" }); result = { ok: true };
      ...
      const sandbox = parseSandboxInput(options.input); result = await executeSandbox({ executionId: options.executionId, userId: options.userId, runtime: sandbox.runtime, code: sandbox.code, input: sandbox.input, limits: sandbox.limits, network: "none" });
    } else {
      const controller = new AbortController(); const timeout = setTimeout(() => controller.abort(), policy.maxToolExecutionMs); const onAbort = () => controller.abort(); options.signal?.addEventListener("abort", onAbort, { once: true });
      try { const toolResult = await executeTool({ userId: options.userId, executionId: options.executionId, toolName: options.toolName, input: options.input, signal: controller.signal, policy }); if (!toolResult.success) throw new Error(toolResult.error ?? `Tool ${options.toolName} failed`); result = toolResult.output; } finally { clearTimeout(timeout); options.signal?.removeEventListener("abort", onAbort); }
    }
