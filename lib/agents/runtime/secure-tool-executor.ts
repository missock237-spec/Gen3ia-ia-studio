...
function parseSandboxInput(input: Record<string, unknown>) {
  ...
  if (runtime !== "node" && runtime !== "python") {
    throw new Error("code.execute requires runtime 'node' or 'python'");
  }

  if (typeof code !== "string" || code.length === 0 || code.length > 500_000) {
    throw new Error("code.execute requires code between 1 and 500000 characters");
  }

  if (providedLimits !== undefined && (typeof providedLimits !== "object" || providedLimits === null)) {
    throw new Error("code.execute limits must be an object");
  }

  const limits = {
    ...DEFAULT_SANDBOX_LIMITS,
    ...(providedLimits as Partial<SandboxLimits> | undefined)
  };

  if (
    !Number.isInteger(limits.timeoutMs) || limits.timeoutMs < 100 || limits.timeoutMs > 120_000 ||
    !Number.isInteger(limits.memoryMb) || limits.memoryMb < 64 || limits.memoryMb > 2_048 ||
    typeof limits.cpu !== "number" || limits.cpu < 0.1 || limits.cpu > 2 ||
    !Number.isInteger(limits.maxOutputBytes) || limits.maxOutputBytes < 1_024 || limits.maxOutputBytes > 10_000_000
  ) {
    throw new Error("code.execute limits are outside the allowed sandbox bounds");
  }

  return {
    runtime: runtime as SandboxRuntime,
    code,
    input: input.input,
    limits
  };
...
export async function executeToolSecurely(options: SecureToolExecutionOptions): Promise<unknown> {
  ...
  if (options.toolName === "code.execute") {
    if (!policy.allowCodeExecution) {
      throw new Error("Code execution is disabled by the execution policy");
    }
