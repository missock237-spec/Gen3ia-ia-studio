...
export async function startLiveRuntime(id: string, deviceId: string) {
  ...
  return adminDb.runTransaction(async (tx) => {
    ...
    const now = Date.now();
    const runtime: LiveRuntimeState = {
      ...(data.runtime ?? { status: "idle", iteration: 0, maxIterations: DEFAULT_MAX_ITERATIONS }),
      status: "running",
      startedAt: data.runtime?.startedAt ?? now,
      error: undefined,
    };
...
export async function recordLiveObservation(id: string, input: {
  deviceId: string;
  ...
  return adminDb.runTransaction(async (tx) => {
    ...
    const next: LiveRuntimeState = {
      ...
      lastDecisionMessage: input.decisionMessage.slice(0, 2000),
      ...(input.actionId ? { lastActionId: input.actionId } : {}),
      ...(input.done ? { completedAt: now } : {}),
      error: undefined,
    };
...
export async function recordLiveRuntimeActionResult(id: string, input: {
  deviceId: string;
  actionId: string;
  ok: boolean;
  error?: string;
  ...
  await adminDb.runTransaction(async (tx) => {
    ...
    tx.update(sessionRef, {
      "runtime.lastActionResult": { ok: input.ok, ...(input.error ? { error: input.error.slice(0, 2000) } : {}), at: now },
      updatedAt: now,
      version: now,
    });
  });
...
export async function claimApprovedLiveAction(id: string, deviceId: string): Promise<LivePendingAction | null> {
  ...
  return adminDb.runTransaction(async (tx) => {
    ...
    if (Date.now() - pending.createdAt > PENDING_ACTION_MAX_AGE_MS) {
      tx.update(sessionRef, { pendingAction: null, status: "paused", "runtime.status": "paused", updatedAt: Date.now(), version: Date.now() });
      return null;
    }
