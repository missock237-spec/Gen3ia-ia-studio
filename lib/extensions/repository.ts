...
export interface ExtensionDoc {
  ...
  developerId: string; developerName: string; status: ExtensionStatus; permissions: string[];
...
export async function createExtension(developer:{userId:string;displayName:string},manifest:ExtensionManifest):Promise<ExtensionDoc>{const ref=extensionRef(manifest.id);const batch=adminDb.batch();const timestamp=now();batch.create(ref,{id:manifest.id,name:manifest.name,description:manifest.description,category:manifest.category,tags:manifest.tags??[],developerId:developer.userId,developerName:developer.displayName.slice(0,120),status:"draft",permissions:manifest.permissions,pricing:manifest.pricing,latestVersion:manifest.version,approvedVersion:null,stats:{installs:0,ratingSum:0,ratingCount:0,executions:0},createdAt:timestamp,updatedAt:timestamp,deletedAt:null});batch.create(versionRef(manifest.id,manifest.version),{id:`${manifest.id}@${manifest.version}`,extensionId:manifest.id,version:manifest.version,changelog:"Initial version.",status:"draft",manifest,submittedAt:null,reviewedAt:null,reviewNote:null,createdAt:timestamp});await batch.commit();const snap=await ref.get();return snap.data() as ExtensionDoc;}
...
export async function listExtensionsByDeveloper(developerId:string){const snap=await adminDb.collection(COL.extensions).where("developerId","==",developerId).where("deletedAt","==",null).orderBy("createdAt","desc").limit(100).get();return snap.docs.map(d=>d.data() as ExtensionDoc);}
export async function softDeleteExtension(id:string,developerId:string){const doc=await getExtension(id);if(!doc)throw new Error("Extension not found.");if(doc.developerId!==developerId)throw new Error("Only the developer may delete this extension.");await extensionRef(id).update({deletedAt:now(),status:"suspended",updatedAt:now()});}
export async function createVersion(developerId:string,manifest:ExtensionManifest,changelog:string):Promise<ExtensionVersionDoc>{const extension=await getExtension(manifest.id);if(!extension)throw new Error("Extension not found.");if(extension.developerId!==developerId)throw new Error("Only the developer may add versions.");const ref=versionRef(manifest.id,manifest.version);await adminDb.runTransaction(async tx=>{const snap=await tx.get(ref);if(snap.exists)throw new Error(`Version ${manifest.version} already exists.`);const timestamp=now();tx.create(ref,{id:`${manifest.id}@${manifest.version}`,extensionId:manifest.id,version:manifest.version,changelog:changelog.slice(0,2000),status:"draft",manifest,submittedAt:null,reviewedAt:null,reviewNote:null,createdAt:timestamp});if(!extension.latestVersion||compareSemver(manifest.version,extension.latestVersion)>0)tx.update(extensionRef(manifest.id),{latestVersion:manifest.version,updatedAt:timestamp});});const snap=await ref.get();return snap.data() as ExtensionVersionDoc;}
