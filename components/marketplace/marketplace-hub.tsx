...
type Installation = { extensionId: string; extension?: Extension; status?: string; version?: string };
...
export function MarketplaceHub() {
  ...
  const visible = useMemo(() => {
    ...
    if (tab === "installed") list = list.filter((item) => installed.some((entry) => entry.extensionId === item.id));
    ...
    <div className="min-h-screen bg-[#060812] text-white">
      <div className="mx-auto flex max-w-[1480px] gap-0 px-3 py-3 sm:px-5 lg:px-7">
        <aside className="sticky top-3 hidden h-[calc(100vh-24px)] w-[248px] shrink-0 flex-col rounded-[28px] border border-white/10 bg-white/[.035] p-4 lg:flex">
          ...
          <nav className="mt-2 space-y-1">{categories.slice(0, 8).map(([id, label, icon]) => <button key={id} onClick={() => { setCategory(id); setTab("discover"); }} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2 text-left text-sm ${category === id ? "bg-white/7 text-white" : "text-white/50 hover:bg-white/5 hover:text-white"}`}><span className="w-5 text-center text-violet-300">{icon}</span>{label}</button>)}</nav>
        ...
        <main className="min-w-0 flex-1 px-2 sm:px-5 lg:px-7">
          <header className="rounded-[30px] border border-white/10 bg-gradient-to-br from-violet-500/[.12] via-white/[.025] to-cyan-400/[.06] p-6 sm:p-8">
            <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between"><div><div className="text-[10px] font-bold tracking-[.28em] text-violet-300">GEN3IA / MARKETPLACE</div><h1 className="mt-3 max-w-3xl text-3xl font-bold tracking-tight sm:text-4xl">Construisez votre environnement IA.</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-white/55">Des extensions professionnelles pour enrichir vos agents avec des tools, skills, workflows et intégrations prêtes à l’emploi.</p></div><div className="grid grid-cols-3 gap-2"><div className="rounded-2xl border border-white/10 bg-black/15 px-4 py-3"><b className="text-lg">{extensions.length}</b><small className="mt-1 block text-[10px] text-white/35">Extensions</small></div><div className="rounded-2xl border border-white/10 bg-black/15 px-4 py-3"><b className="text-lg">{favorites.length}</b><small className="mt-1 block text-[10px] text-white/35">Favoris</small></div><div className="rounded-2xl border border-white/10 bg-black/15 px-4 py-3"><b className="text-lg">∞</b><small className="mt-1 block text-[10px] text-white/35">Possibilités</small></div></div></div>
